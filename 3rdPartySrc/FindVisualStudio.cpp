...
// Added VS and SDK include paths, various other paths
// Created header file
...
void find_windows_kit_root(Find_Result *result) {
    ...
    if (windows10_root) {
        ...
        if (data.best_name) {
			...
			memcpy(result->windows_target_platform_version, data.best_version,
			       sizeof(result->windows_target_platform_version));
		}
